...
- 干饭人和做饭人是可切换的工作模式，不是普通用户和管理员的产品概念。
- 两种身份共用首页、菜单、菜谱和我的页面。
- 只有做饭人可以配置菜单/菜谱，以及推进订单制作流程。
...
- `admin` 当前是底部 Tab，二期应改为做饭人模式下的配置入口。
...
`menu-config` 和 `recipe-config` 不进入底部导航，只能从做饭人模式下的菜单页或菜谱页进入。
...
产品文案统一使用“干饭人/做饭人”，代码内部建议使用稳定的 `diner/cook` 枚举。旧的 `orderer/admin` 只作为迁移兼容值，不再作为新功能的产品语义。
...
二期第一阶段中，`options` 可以先记录菜品点单时的选项；是否将菜谱口味选择同步为订单选项，需要单独确认后再启用。

### 5.5 现有 dishes 的迁移策略

当前 `dishes` 同时承担菜单项和菜品基础信息。建议迁移方式：

1. 新增 `families`、`recipes` 和 `menu_items` 数据结构。
2. 为现有 `dishes` 生成对应的历史菜谱记录。
3. 为现有菜单创建对应的 `menu_items` 关联。
4. 将旧菜品标记为待完善菜谱，避免无做法内容被误认为完整菜谱。
5. 菜谱和菜单新功能上线后，停止新写入旧 `dishes`。
6. 现有菜单是否在菜谱补全前继续展示，需要根据实际数据量决定；默认建议先完成迁移检查，再切换点单入口。

如果当前线上没有需要保留的真实菜品数据，也可以直接按新模型初始化，减少兼容代码。
...
现有 `dishes` 没有完整菜谱。上线前必须决定是迁移为待完善菜谱、隐藏旧菜单，还是保留兼容展示。不能让新菜单规则和旧数据产生隐性绕过。

### 身份与权限风险

身份可随时切换意味着“做饭人”是工作模式，而不是固定账号权限。后端必须明确这是产品规则；若未来需要固定权限，要新增独立能力字段。
...
确认后再进入阶段 1 的动漫手绘视觉方案和阶段 2 的应用壳重构。
