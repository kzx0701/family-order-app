...
export const cartKey = (id, options) => JSON.stringify([id, options])
export function addToCart(lines, item, options = item.defaults, quantity = 1) {
  const key = cartKey(item.id, options)
  const line = lines.find(value => value.key === key)
  const current = line?.quantity || 0
  if (current + quantity > 20) return false
  if (line) line.quantity += quantity
  else lines.push({ key, id:item.id, recipeId:item.recipeId, name:item.name, image:item.image, options:[...options], quantity })
  return true
}
export function decreaseLine(lines, key) {
  const index = lines.findIndex(item => item.key === key)
  if (index < 0) return
  if (lines[index].quantity > 1) lines[index].quantity--
