...
const utils_handFont = require("./utils/hand-font.js");
if (!Math) {
...
const _sfc_main = {
  onLaunch(options) {
    common_vendor.index.__f__("log", "at App.vue:21", "[App] onLaunch", options);
    this.bootstrap();
    utils_handFont.loadHandFont();
  },
  onShow() {
    common_vendor.index.__f__("log", "at App.vue:38", "[App] onShow");
    if (!bootstrapped)
      return;
    utils_authGuard.ensureAuth({ silent: true });
  },
  onHide() {
    common_vendor.index.__f__("log", "at App.vue:46", "[App] onHide");
  },
  methods: {
    async bootstrap() {
      try {
        ...
        common_vendor.index.__f__("error", "at App.vue:60", "[App] bootstrap error", e);
      } finally {
        bootstrapped = true;
        utils_authGuard.ensureAuth({ silent: true });
      }
