...
const iconPaths = {
  ...
  // 菜单（一张写着条目的单子）—— **线条版**（2026-09-23 由实心版换过来，主人要求"清爽的、线条的"）。
  // 用法：菜谱卡片右上角那枚绿色状态标记（`.card-status`，元素 34rpx / `:size="17"`）。
  // ⚠️ 尺度经验（两轮实测的结论，别再反复）：
  //    · 元素 **≥ 15px（30rpx）时线条是可行的**，而且比实心清爽；
  //    · 但线条的"视觉重量"比实心小 —— **压在深色菜品照片上会明显更弱**（这一条是主人知情后接受的代价）；
  //    · **别把元素缩到 15px 以下**（线条会糊成一片），也**别为了"更结实"自行改回实心**（形态是主人定的）。
  // 写法是常规的 lucide 线条：不写颜色、描边由组件统一给。
  'menu-slip': '<rect x="4.5" y="3" width="15" height="18" rx="2.4"/><path d="M8.3 9.2h7.4"/><path d="M8.3 14.8h7.4"/>',
  // 咖啡
