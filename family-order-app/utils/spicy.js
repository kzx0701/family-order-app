 ...
 *
 * 两种展示语义要分开，别混：
 * - **字段**（点单抽屉的辣度行：只画图标、不再写文字）—— 四档都要画出来，包括不辣的那枚
 *   斜线辣椒，因为这一行**图标就是值**，而且"不辣"是一个要读出来的值；
 ...
 * 【尺寸换算须知 —— 改辣度图案尺寸前必读】
 *
 * 四张素材都是 `viewBox="0 0 96 96"` 的方形画布，而辣椒主体只占画布中间一块。
 * **下面是实测边界**（2026-09-21：把 4 张 SVG 各渲成 384×384 位图、取 alpha 包围盒得来；
 * x / y 均为 96 画布内的坐标。改素材后必须重新量一遍，下面的补偿值全部由它推出）：
 *
 *   档位   内容 x         内容 y       内容宽   内容高   左留白   右留白
 *   不辣   24.0 → 71.0   20.0 → 73.0    47.0     53      24.0     25.0
 *   微辣   35.8 → 61.0   20.0 → 73.0    25.2     53      35.8     35.0
 *   中辣   20.8 → 77.0   20.0 → 73.0    56.2     53      20.8     19.0
 *   特辣    5.8 → 93.0   20.0 → 73.0    87.2     53       5.8      3.0
 *
 * 两个直接结论：
 *
 * ① **纵向**：内容高恒为 53/96 = **0.552**，上留白 20、下留白 23（**两者不等**，内容中心
 *    比画布中心高 1.5 单位）。所以 `aspectFit` 装进边长 S 的方框时，辣椒实际只有 `S × 0.552` 高：
 *
 *      想让辣椒看起来高 H  →  方框给 `H ÷ 0.552 ≈ H × 1.81`
 *
 * ② **横向：留白随档位剧烈变化**（微辣左右各 35 单位，特辣只有 3~6 单位，差 30 单位）。
 *    所以凡是要把辣椒**贴到某条边上**的展示位，必须逐档抵掉这段留白 —— 否则同一列卡片
 *    里「微辣」比「特辣」离边多出 30 单位（46rpx 盒子约 14rpx），看着就是不齐的。
 *
 ...
 *
 * 「与文字并排」的展示位统一取「辣椒视觉高 ≈ 24rpx」，即方框 `24 ÷ 0.552 ≈ 43.5 → 46rpx`
 * （46rpx 实得 **25.4rpx**，与 23rpx 的行文字基本等高，故未改回 44rpx）。
 * **点单抽屉的辣度行是唯一例外**：2026-09-21 改版后它「图标即值」（不再有文字并排），
 * 放大一档到 56rpx（实得 30.9rpx）。各展示位的尺寸与换算对照（便于回滚核对）：
 *
 *   - 菜谱列表卡片：旧 `:size="13"`（26rpx 元素）→ 辣椒实际 22.4rpx → 方框 46rpx
 *   - 菜谱详情浏览态：旧 `size="28rpx"` → 辣椒实际 24.2rpx → 方框 46rpx
 *   - 点单抽屉只读行：旧 `:size="15"`（30rpx 元素）→ 辣椒实际 25.9rpx → 方框 46rpx
 *     → 2026-09-21 改「图标即值」后 **56rpx**
 *   - 点单菜单卡片：方框 36rpx（那一页字号体系比菜谱页小一档，辣椒实得 19.9rpx）
 *
 * 其余几处取同一个值而不是各拍一个：**「统一样式」的要义就是同一档辣度在任何页面都一样大**，
 * 而且素材辣椒比旧 icon 瘦（宽高比 0.47 vs 0.58），视觉重量轻，略放大一点反而更接近。
 * 点单抽屉之所以破例，理由只有一条：那里图标不再与文字并排、得自己承担"值"的视觉重量；
 * 一旦给它旁边补回文字，尺寸就该退回 46rpx。
 *
 * 【两处「抵掉透明留白」的补偿 —— 目的都是让图标的**光学**边缘落在该落的位置】
 *
 *   · **纵向 · 点单抽屉**（pages/order/order.vue 的 `.spicy-art`）：`margin-top:-13rpx`。
 *     不抵的话「字段标题→内容」的光学间距会比同页文字块松近一倍。
 *     ⚠️ 按实测上留白 20/96，56rpx 盒子的精确值应是 **−11.7rpx**；现用的 −13rpx 是早先
 *     按「上下各 13rpx」的对称假设算的，偏了 1.3rpx ≈ 0.65px（肉眼不可辨，故未动它）。
 *
 *   · **横向 · 菜谱列表卡片**（pages/recipe/recipe.vue 的 `.card-spicy`）：**逐档**给左右各
 *     一份负外边距，让「图标可视框 = 图标布局框」——这样同一个 flex 间隙（14rpx）与同一个
 *     右端内边距（18rpx）对每一档都成立，卡片右列才真正对齐。
 *     数值 = 左右留白均值 ÷ 96 × 方框边长（46rpx）：微辣 **−17rpx**、中辣 **−9.5rpx**、
 *     特辣 **−2rpx**。「不辣」在卡片上根本不渲染（`spicyMark` 语义），故无此值。
 *
