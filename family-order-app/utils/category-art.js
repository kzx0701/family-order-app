 ...
 * 菜品分类图标：**唯一的事实来源**
 *
 * 调用方：菜谱列表页顶部的分类栏、点单页顶部的分类栏、**菜谱编辑页的分类选择抽屉**、
 * 菜谱详情页编辑态的分类选择行与前导图、点单页菜品卡片上的做法分类小图。加一个分类、换一张素材，只改这里。
 *
 ...
 * ⚠️ 六个动图都在**云存储**上（不是本地文件）。**进页面不会下载它们**：
 *    动图层只在某一格**被选中**时才挂载 —— 列表页分类栏、点单页分类栏、菜谱编辑页的分类抽屉
 *    **三处同一套口径**：选中哪一格，哪一格才加载；静态图一直露着，直到动图 `load` 到达才让位
 *    （动图加载失败就维持静态图，不会出现空白格）。
 *    ⚠️ 曾经还有一层 `v-for` 把**全部**动图一次性挂上预热的机制，六个配满后是无差别的 773KB，
 *    **2026-09-22 已删** —— 别再照那条旧做法加回来。
 *
